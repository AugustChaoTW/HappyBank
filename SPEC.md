...
| active | bool | |
| description | string | **做法說明**：一句寫給小孩看的、具體到他自己檢查得完的標準。可留空 |

> ⚠️ **`description` 一定要接在最後一欄**（`active` 之後），不要照上表的排版插在中間。
> 理由同 `requests`：`setup()` 只重寫表頭那一列、不搬動任何資料——
> 插在中間會讓 Sheet 上既有的每一列默默錯位一格（舊的 `active` 會被讀成 `description`）。
>
> ⚠️ **既有的家事不會自動長出說明。** `seedTable()` 只補「第一欄的鍵不存在」的**整列**，
> 不會回頭替既有列填新欄位。所以線上 `chores` 分頁裡那六件（或家長自己加的）
> `description` 會是**空白**，要家長**自己打進 Sheet**。`CHORES_SEED` 裡的預設說明
> 只對「全新的、還沒有這個 id 的」那一列生效。

#### 為什麼要有做法說明

「倒垃圾」在大人腦裡跟在六歲小孩腦裡不是同一件事。沒有一句寫下來的定義，
小孩只能猜——他拍一張「一個垃圾桶」的照片送出，被退回，
於是他學到的是「**這系統很任性**」，而不是那個標準本身。

家長寫一次的一行字，換掉的是每週一次的退回。所以說明要**具體、可檢查、寫給小孩看**：
「廚房跟浴室兩個桶子都要倒，倒完記得換新袋子」，而不是「把垃圾處理好」。
...
- **chores**：**每日家事清單**。一件家事一張卡：icon、名稱、獎金、以及一個**狀態**——
  ...
  **名稱底下一行「做法說明」**（`chores.description`，§5）：字比標題小、顏色更淡，
  明確**從屬於**家事名稱——小孩掃過去先看到「倒垃圾」，需要的時候才讀那一行標準。
  **說明是空的（家長還沒打）就整行不畫**：不留空框、不留孤單的破折號或冒號。
  那是家長打進 Sheet 的自由文字，會直接進 `innerHTML`，**一律跳脫**（`<` 打進去就顯示 `<`，不是跑成標籤）。
...
- **admin**：僅 `role = parent` 的 session 可進入（伺服器判定，前端只是隱藏入口）。
  ...
  **待審清單要分得出兩種 kind**，不能混成一團看不出誰是誰：
  - 每一筆前面掛一個明顯的類型標記——每日簽到 `🌞 每日零用錢`（**本行原本寫「🌞 每日簽到」，
    以程式為準**：小孩端的按鈕、ledger 的 memo 都寫「每日零用錢」，家長端不該再冒出第三種講法）、
    家事 `🧹 <家事名稱>`。
  - **每日簽到那筆要把勾選內容攤開**（讀 `requests.checklist`，三個項目逐條列出來打勾），
    媽媽要看的是「他說他做到了什麼」＋照片，不是一個光禿禿的「簽到」兩個字。
    `checklist` 解析不出來（空的、格式怪）時寫**「（沒有勾選紀錄）」**，
    不要留一片空白，更不要讓整張清單炸掉——那是媽媽正在核准的證據。
  - **金額顯示也不同**：家事是 `chores.reward`，每日簽到是該小孩的 `users.dailyAllowance`
    （家長端 snapshot 的 `kids[].dailyAllowance`，§7.1）。兩者都由伺服器算，前端只是顯示。
    **`requests.amount` 在 pending 階段是空的，不能拿來當金額**；查不到金額時只寫
    「🌞 每日零用錢」不寫數字——寫錯的數字比不寫更糟。
  - **家事那筆也要印出做法說明**（`chores.description`，§5）：媽媽在判照片的時候，
    要看得到**她自己當初訂的那句標準**，否則她只能憑當下的印象判，
    小孩被退回的理由就會每週不一樣。放在小孩名字那一行底下，**壓到最小可讀**——
    待審清單很密，一筆一行就好；空的就不畫那一行。同樣要跳脫。
    ⚠️ 舊快照沒帶 `chores` 時，前端的寫死對照表**刻意不含 description**：
    做法說明是家長隨時會改的字，拿寫死的舊值給正在核准的人看，比不給更糟
    （同上面對金額的處理）。
  - **縮圖要不要抓看的是 `photoFileId`，不是 kind**：兩種 kind 都有照片，
    以後多一種也不必再改一次判斷式。
  - **全選核准一律「按類型」**：「全部核准 每日零用錢（3 筆）」是週日對帳日最常按的一顆鈕（§9.4）。
    **不提供跨 kind 的一鍵全選**——簽到看的是勾選紀錄、家事看的是照片，證據不同，
    一顆鈕同時放行兩種等於讓媽媽核准她根本沒看過的那一種。
    同一類少於兩筆時不長出全選鈕（按那一列就好）。
    全選核准仍然逐筆、**序列**走 `admin_decide`，代理規則一樣要過（§9.5）。
...
{ "ok": true, "serverTs": "...", "today": "2026-09-18", "rates": {...},
  "user": { /* 家長自己 */ },
  "kids": [ { "user": {...}, "accounts": [...], "total": 1234, "dailyAllowance": 20 } ],
  "chore_approver": "vicky",
...
- 登入者一律放在 **`user`**（不是 `kid`）；家長的小孩清單放在 `kids`，
  每個元素是 `{ user, accounts, total, dailyAllowance }`，`total` 是該小孩所有未關閉帳戶餘額加總，
  `dailyAllowance` 是該小孩那一列的每日零用錢（整數元，沒填就是 `0`）。
  **家長端要逐人帶這個值**：待審清單上 `allowance_claim` 那筆在 `pending` 階段沒有金額可看
  （見本節最後一條），沒有它前端只能猜一個數字。
...
- 家長端的 `requests` 帶 `photoFileId`（**不是圖片內容**，snapshot 不塞 base64）供待審清單
  ...
  **待審階段的 `req.amount` 不要拿來當金額用**。
  查的地方就是同一份 snapshot 的 `kids[].dailyAllowance`（上面第一條）。
...
- **照片是拿來對照「做法說明」判的**（`chores.description`，§5）。
  那一行字就是**這張照片要被判成通過的標準**：家長寫一次、小孩做之前看得到、
  家長核准時同一句話也印在待審清單上（§6）。三個地方是同一份文字，
  所以「做到什麼程度算做完」在事前事後都是同一個答案——
  退回的時候講的也是那條標準，不是當下的心情。標準沒寫（空白）不會擋住流程，
  只是又回到「小孩用猜的」那個狀態。
...
- [ ] **家事卡上看得到做法說明**：家事名稱底下一行小字寫出 `chores.description`，
      比標題小、比標題淡；**待審清單的家事那筆也印得出同一句**
- [ ] **做法說明是空的時候整行不畫**：卡片與待審列都不留空白框、不留多餘的間距或標點
- [ ] **說明裡打 `<b>粗體</b> & "引號"` 會照字面顯示**，不會被當成 HTML 跑掉（小孩端與家長端都是）
- [ ] **`setup()` 不會回頭補既有列的說明**：跑過 `setup()` 之後 `chores` 分頁多一個
      `description` 欄、但既有六列是空白的，要家長自己打；**既有列的其他欄位一格都沒錯位**
...
- [ ] 家長端待審清單**分得出兩種 kind**：簽到那筆寫「🌞 每日零用錢」＋該小孩的 `dailyAllowance` 金額
      （**不是 pending 階段那個空的 `req.amount`**）、攤開的勾選項目與照片；家事那筆寫家事名稱與 `chores.reward`
- [ ] 待審清單上**兩種 kind 都看得到照片縮圖**（判斷依據是 `photoFileId`，不是 kind）；
      `checklist` 是空的或壞掉時那一列寫「（沒有勾選紀錄）」，**清單不會炸掉**
- [ ] **「全部核准 每日零用錢（3 筆）」一次批掉三筆**，而且**只批那一類**——
      家事那幾筆不會被同一顆鈕放行；同類少於兩筆時不出現全選鈕
