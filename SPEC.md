...
|---|---|---|---|---|
| 1 | **每週零用錢** | 系統自動 | 爸媽給的固定金額，**每人不同**，存在 `users.weeklyAllowance`（§5）。**每週日早上 8:00** 由 time-driven trigger 自動入帳（§4.5、§11 M3），人不能插手（§9.2） | `allowance` |
| 2 | **家事獎金** | 小孩回報 → 家長確認 | 回報家事**＋照片**，經 `chore_approver`（預設 Vicky，或代理）確認後入帳；**金額由伺服器查 `chores.reward`**，前端改金額沒用（§9.5） | `chore` |
| 3 | **從子帳戶轉回來** | 小孩自己 | 定存到期或提前解約、儲蓄目標達標後，把**本金（及已累積的利息）**轉回活期。免審，按下去就成立（§9.2）。解約的話利息已先被回沖（§4.3） | `transfer_in` |
...
> 這三個數字**取代先前暫定的全家一律 50**。理由見 §4.5：金額壓低，
> 利息才會成為「錢變多」的主要感覺來源。
>
> **這些值是資料，不是程式碼**：存在 `users.weeklyAllowance` 欄位，
> 直接在 Sheet 的 `users` 分頁改（或跑 `Setup.gs` 的 `upsertUser()` helper），
> 改完即時生效，不需要重新部署。家長端 admin 也改得動（§6）。
>
> ⚠️ `apps-script/Setup.gs` 的 `seedUsers()` **目前仍寫死 50**，
> 只影響「第一次建帳號」，不影響已存在的資料。待辦見 §11。
...
- 小孩人數與名單、年齡（影響 UI 用字）
- ~~每個小孩的 `weeklyAllowance` 金額（逐人設定）~~ ✅ **已定案**：Momo 20 / Coco 15 / Dodo 10（見 §5）
- 家事清單初始內容與定價
- 🔴 **冷啟動：頭幾個月「錢不會變多」的風險（開帳金？）**
  金額定案後把數字算出來，結論不太妙。以每月 1 日結息、`Math.round` 不足 1 元記 0（§3）計算，
  **只靠零用錢、完全不花錢**存滿一年（52 週、12 次結息）：

  | 小孩 | 每週 | 一年零用錢 | 一年利息 | 一年後活期餘額 | **第一次結息** |
  |---|---|---|---|---|---|
  | Momo | 20 | 1,040 | 341 | **1,381** | **+4 元** |
  | Coco | 15 | 780 | 255 | **1,035** | **+3 元** |
  | Dodo | 10 | 520 | 172 | **692** | **+2 元** |

  （Momo 每週 20 ≈ **每月 87 元**；Dodo 每週 10 ≈ **每月 43 元**。
  一年下來利息約佔餘額的四分之一，長期是有感的。）

  問題在**開頭**：利息一個月才結一次，所以**第一個月整整四週，小孩看到的利息是 0 元**；
  等到第一次結息，Momo 拿到 4 元、Dodo 只有 2 元。而 §3 已經寫明
  「餘額低於 20 元的活期帳戶幾乎沒有利息」——一開始餘額本來就在那個區間。
...
- 🔧 **`apps-script/Setup.gs` 的 `seedUsers()` 還停在舊的預設值 50**，
  要改成 §5 已定案的 Momo `20` / Coco `15` / Dodo `10`。
  這只影響「從零建帳號」那一次；若 `users` 分頁已經有資料，
  正確做法是**直接在 Sheet 改那一欄**，不要為了改金額重跑 seed。
  順帶檢查 `CONFIG_SEED` 有沒有殘留零用錢相關的 key——**不該有**，
